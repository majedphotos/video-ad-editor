...
📱 **الاستوديو (v3.1) — جوال أو متصفح:** تايم لاين باللمس فوق نفس المحرّك ونفس الراسم (صفر توكنز لكل تعديل). فيه: التحكم بالقص (رجّع أو اقصّ السكتات، احذف مقطعاً أو جملة — يعيد البناء ويحوّل كل الأزمنة) · 6 ستايلات كابشن (كرت · كبير بلا خلفية · كلمة وحدة ضخمة · خط تحت الكلمة · شريط بلون التمييز · ماكينة كاتبة) · مؤثر على أي كلمة (طيحة · اهتزاز · كتابة · نبضة) · «ورا الراس» بضغطة · مؤثر صوتي عند الجملة · رفع بي-رول من الجوال (ملء الشاشة أو كرت فوق) · المشاهد الأربعة + الكولاج · الهوية · مناطق أمان انستقرام · قبل/بعد · رابط للعميل يعلّق على أي ثانية (`/share`) · التصدير من الجوال. التعديلات تُحفظ بـ`studio.json` (+caps/theme/sfx) وتنعكس بأي رسم لاحق. **⛔ بمشاريع الاستوديو مشاهد الرسم المكتوبة بـcompose.html مطفية** إلا لو حطّيت `sceneFx:true` بـstudio.json (لما تكتب مشاهد مخصصة لهذا الفيديو).
المسافة = تشغيل/إيقاف · الأسهم = فريم · Shift+سهم = ثانية.
```bash
...
### 7.9) 🖼️ مشاهد مولّدة — صورة أو مقطع يتولّد من كلامه والمحرّك يرسم فوقه (v3.2)
...
1. `director_entitlements` (مجاني) — يرجّع باقته ورصيده. **ما عندك أدوات `director_*` أصلاً (ولا نجح `director_entitlements`)؟ معناها الخادم مو مربوط** — شغّل `bash scripts/00_setup.sh --director-check` (فحص الربط بعلم مستقل لأنه يوصل 23 ثانية، فانشال من الخطوة 0) — يرجّع «مربوط» أو يطبع أمر الربط. **«مربوط» بس ما عندك الأدوات؟** الخادم مسجّل بلا مصادقة أو يحتاج إعادة تشغيل: قل له «اكتب `/mcp` ← `director` ← Authenticate، أو أعد تشغيل Claude Code» ثم أعد `director_entitlements`. طبع الأمر؟ جملة وحدة للمستخدم: «أربطك بعيون المخرج عشان أولّد لك المشاهد بحسابك — تسجّل دخولك من المتصفح مرة وحدة، أبدأ؟» وبعد موافقته شغّل الأمر كما هو (`claude mcp add --transport http director https://hawsh-khalifa.com/mcp …`). **الأمر يسجّل الخادم وبس — ما يدخّله ولا يفتح متصفح**، فقل له بعده جملة وحدة: «اكتب `/mcp` ← اختر `director` ← Authenticate — يفتح المتصفح مرة وحدة وتسمح؛ ولو ما ظهر `director` بالقائمة أعد تشغيل Claude Code وشغّل «منتج هذا المقطع» من جديد.» وبعدها أعد `director_entitlements`.
...
```
7. **الرفض: جملة وحدة وبس، وما ينخصم شي بأي حالة منها** (الرفض يجي قبل الخصم):
   - `no_access` (ما عنده رصيد ولا اشتراك): «رصيدك خلص — باقة 25 نقطة بـ10 د.ب من هالرابط، وبعدها أكمّل من نفس المكان: https://hawsh-khalifa.com/tools/director?from=video-ad-editor».
   - `agent_cap` (سقف صرف الوكلاء اليومي): «تجاوزنا سقف اليوم — نكمّل باچر». ⛔ لا تعطيه رابط الشراء هني، شراء النقاط ما يرفع سقف اليوم.
   - أي رفض ثانٍ (`content_blocked` · `expired` · `blocked`): قل السبب بجملة وحدة، عدّل الوصف مرة وحدة، ولا تكرّر.
8. **منتج حقيقي بالمشهد:** صورة منتجه تدخل مرجعاً بدل ما توصفه بالكلام — `director_import_reference` (رابط https عام، حتى 25 ميقا) يرجّع `path`، مرّره بـ`reference_paths` للصورة و`references:[{"path":"…"}]` للمقطع، وبالمقطع اربطها بالبرومبت بـ`@Image1` و`@Image2`… بالترتيب. **صورته على جهازه؟ ما ينفع** — اطلب منه رابط الصورة من موقعه أو متجره بجملة وحدة، وإلا وصفها بالكلام. **ولقطات التطبيق تبقى من موقعه الرسمي** كما هي القاعدة فوق.
...
| | يسوي شنو | المحرّك |
|---|---|---|
| `00_setup.sh` | يفحص الأدوات وينزّل الناقص · `--director-check` يفحص ربط عيون المخرج (بطيء، بعلم مستقل) | مشترك |
